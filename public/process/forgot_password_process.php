...
app_debug_log('identifier=' . $identifier . ' | user_found=' . ($user ? 'yes(id=' . $user['id'] . ',status=' . $user['status'] . ')' : 'no'));
...
try {
    if ($user && $user['status'] === 'active') {
        ...
        app_debug_log('password_resets insert OK for user_id=' . $user['id']);
        ...
        $mailSent = send_app_mail($user['email'], 'Recuperá tu acceso a ' . APP_NAME, $body);
        app_debug_log('send_app_mail returned ' . ($mailSent ? 'true' : 'false') . ' (SMTP_HOST=' . (SMTP_HOST !== '' ? SMTP_HOST : 'not configured, using native mail()') . ')');
    }
} catch (Throwable $e) {
    $detail = $e->getMessage();
    ...
    app_debug_log('EXCEPTION: ' . $detail);
}
