...
$conversations = ContactMessage::liveChatConversations();
?>
...
<h2 class="h5 fw-semibold mb-3"><i class="bi bi-headset"></i> Chat en vivo (widget)</h2>
        ...
        <tbody>
            <?php foreach ($conversations as $conv): ?>
                <tr class="<?= $conv['unread_count'] > 0 ? 'fw-semibold' : '' ?>">
                    ...
                    <td class="text-end">
                        <a href="<?= BASE_URL ?>/mensaje_chat.php?email=<?= urlencode($conv['email']) ?>" class="btn btn-sm btn-primary">Abrir chat</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            <?php if (!$conversations): ?>
                <tr><td colspan="7" class="text-center text-secondary py-4">No hay conversaciones de chat en vivo todavía.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
